    ...
    private function initializeBtnActions() {
        if ($this->settings["actionbtn"]) {
            if ($this->settings["viewbtn"]) {
                $this->enqueueBtnActions("view btn btn-info btn-sm", "javascript:;", "view", "<i class=\"fa fa-search\"></i>", "", array("title" => $this->langData["view"]), "btn-info");
            }
            ...
            if ($this->settings["inlineEditbtn"]) {
                $this->enqueueBtnActions("inline_edit btn btn-warning btn-sm", "javascript:;", "inline_edit", "<i class=\"fa fa-pencil\"></i>", "", array("title" => $this->langData["inline_edit"]),"btn-warning");
            }
            if ($this->settings["delbtn"]) {
                $this->enqueueBtnActions("delete btn btn-danger btn-sm", "javascript:;", "delete", "<i class=\"fa fa-times fa-fw\"></i>", "", array("title" => $this->langData["delete"]),"btn-danger");
            }
            if (isset($this->settings["clonebtn"]) && $this->settings["clonebtn"]) {
                $title = isset($this->langData["clone"]) ? $this->langData["clone"] : "sffd";
                $this->enqueueBtnActions("clone btn btn-warning btn-sm", "javascript:;", "clone", "<i class=\"fa fa-clone\"></i>", "", array("title" => $title),"btn-warning");
            }
        }
    }
...
//        unset($this->btnActions["edit"]);
    ...
    private function getSidebarData($data, $result) {
        ...
        $data = $this->getDiv("pdocrud-main-content", array("class" => "col-md-8 pdocrud-main-bar"), $data);
    ...
    public function getSubmitField($fieldName, $attr = array(), $data = array(), $fieldClass = array()) {
        $class = implode(" ", $fieldClass);
        $formId = $this->formId;
        $field = "<input data-form-id=\"$formId\" type=\"submit\" class=\"btn btn-success pdocrud-form-control pdocrud-submit btn-sm $class\" id=\"$fieldName\" name=\"$fieldName\" ";
        if (is_array($attr) && count($attr)) {
            foreach ($attr as $c => $v) {
                $field .= " $c=\"$v\" ";
            }
    ...
    public function getButtonField($fieldName, $attr, $data = array(), $fieldClass = array()) {
        $class = implode(" ", $fieldClass);
        $formId = $this->formId;
        $field = "<button data-form-id=\"$formId\" type=\"button\" class=\"btn btn-danger pdocrud-form-control pdocrud-button btn-sm $class\" id=\"$fieldName\" name=\"$fieldName\" ";
        $buttonText = "";
